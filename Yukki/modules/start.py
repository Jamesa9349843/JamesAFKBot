...
async def on_start(_, message: Message):
    ...
    send = await message.reply_photo(image, caption=f"Hello! My name is {botname}.\n\nTo know more about me check help section. Active since {Uptime}". \n\nJoin: @blackpinkofficial_groupchat, reply_markup=upl)
...
async def on_private_start(_, message: Message):
    if len(message.text.split()) > 1:
        ...
        image = random.choice(RANDOM)
        await message.reply_photo(image, caption=f"Hello! My name is {botname}.\n\nTo know more about me check help section by /help. Active since {Uptime}.\n\nJoin: @blackpinkofficial_blinks", reply_markup=upl)
